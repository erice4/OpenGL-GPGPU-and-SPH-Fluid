...
//currently crashes if too many particles are added, so is commented out
void ParticleEmitter::setTotalParticles(int _numParticles)
{
/*
  if (_numParticles < m_particles.size())
  {
    int diff = m_particles.size() - _numParticles;
  ...
  {
    int diff = _numParticles - m_particles.size();
    ...
    for (int i=0; i<diff; i++)
    {
      m_particles.push_back(createParticle(ngl::Vec3(distributionX(generator),distributionY(generator),distributionZ(generator))));
    }
  }

  m_maxParticles = m_particles.size();
  std::cout<<"new particle number: " << m_particles.size() << "\n";*/
}
...
void ParticleEmitter::draw(ngl::Mat4 _VP, ngl::Vec3 _cameraPosition)
{
  ...
  for(int i=0; i<m_maxParticles; ++i)
  {
    if (m_particles[i].m_active)
    {
      ...
      p.color = ngl::Vec3(0.55f,0.75, m_particles[i].m_density);
